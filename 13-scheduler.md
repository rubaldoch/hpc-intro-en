...
```bash
[yourUsername@sorgan ~]$ nano example-job.sh
```
...
```bash
[yourUsername@sorgan ~]$ bash example-job.sh
```

```output
This script is running on sorgan
```
...
```bash
[yourUsername@sorgan ~]$ sbatch  example-job.sh
```
...
13935 cpu       example-   user01  R       0:05      1 sorgan-cpu6
```
...
```bash
[yourUsername@sorgan ~]$ cat example-job.sh
```
...
```output
JOBID PARTITION     NAME     USER ST       TIME  NODES NODELIST(REASON)
13937 cpu       hello-wo   user01  R       0:02      1 sorgan-cpu6
```
...
```bash
[yourUsername@sorgan ~]$ cat slurm-13937.out
```

```output
This script is running on ...
slurmstepd-sorgan-cpu: error: *** JOB 13937 ON sorgan-cpu6 CANCELLED AT 2026-03-17T23:08:48
DUE TO TIME LIMIT ***
```
...
```output
Submitted batch job 13938

JOBID PARTITION     NAME     USER ST       TIME  NODES NODELIST(REASON)
13938       cpu long_job   user01  R       0:02      1 sorgan-cpu6
```
...
```bash
[yourUsername@sorgan ~]$ scancel 13938
...
```bash
[yourUsername@sorgan ~]$ scancel -u yourUsername
```
...
```bash
[yourUsername@sorgan ~]$ srun hostname
```
...
```bash
[yourUsername@sorgan ~]$ srun -n 2 echo "This job will use 2 CPUs."
```
...
```bash
[yourUsername@sorgan ~]$ srun --pty bash
```
...
connect with this feature enabled, use the `-X` option when you login with
the `ssh` command, e.g., `ssh -X -p 6868 yourUsername@ssh.lsc.ic.unicamp.br`.
