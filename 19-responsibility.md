...
## Be Kind to the Login and Head Nodes

The login node is often busy managing many simultaneous users, while the head node 
is typically used for tasks such as creating and editing files or compiling software. 
If either machine runs out of memory or processing capacity, it can become slow or 
unusable for everyone. These systems are meant to be used—but always do so responsibly,
 in ways that do not negatively impact other users.

Head nodes are generally the appropriate place to prepare and launch jobs. 
Cluster policies may vary, but they are often used to prototype workflows 
and, in some cases, to access advanced debugging or development tools specific 
to the cluster. You may also need to load modules (sometimes in a specific order) 
or work with paths and library versions that differ from your local machine. 
Running small interactive test jobs on the head node is a quick and reliable way 
to identify and resolve these issues before submitting larger jobs.

:::::::::::::::::::::::::::::::::::::::::  callout

## Login and Head Nodes Are a Shared Resource
...
you're contemplating is suitable for the login/head node, or if there's another
...
are running on the login/head node along with the amount of CPU and memory they are
using. If this check reveals that the head node is somewhat idle, you can
...
## Login/Head Node Etiquette

Which of these commands would be a routine task to run on the head node?
...
If you experience performance issues with a login or head node you should report it to
...
best way to transfer them to Sorgan?

1. `scp -p 6868  -r data yourUsername@ssh.lsc.ic.unicamp.br:~/`
2. `rsync --port=6868  -ra data yourUsername@ssh.lsc.ic.unicamp.br:~/`
3. `rsync --port=6868  -raz data yourUsername@ssh.lsc.ic.unicamp.br:~/`
4. `tar -cvf data.tar data;`

   `rsync --port=6868  -raz data.tar yourUsername@ssh.lsc.ic.unicamp.br:~/`
5. `tar -cvzf data.tar.gz data;`

   `rsync --port=6868  -ra data.tar.gz yourUsername@ssh.lsc.ic.unicamp.br:~/`
