...
hpc-carpentry-amdahl-d5a3bb9/requirements.txt
hpc-carpentry-amdahl-d5a3bb9/setup.py
```
...
```bash
[you@laptop:~]$ mv hpc-carpentry-amdahl-d5a3bb9 amdahl
```
...
```bash
[you@laptop:~]$ scp -P 6868  local_file yourUsername@ssh.lsc.ic.unicamp.br:remote_destination
```
...
```bash
[you@laptop:~]$ scp -p 6868  amdahl.tar.gz yourUsername@ssh.lsc.ic.unicamp.br:
```

::: callout
## Shared home directory on Sorgan

On Sorgan the login node and the head node share the same `$HOME` directory. This means that any file you upload to your home directory on the login node will be available on the head node, and vice versa. This is not always the case on other clusters, so be sure to check with your system administrators if you are unsure about how your cluster is configured.

:::

:::::::::::::::::::::::::::::::::::::::  challenge

## Why Not Download on Sorgan Directly?
...
```bash
[you@laptop:~]$ ssh yourUsername@ssh.lsc.ic.unicamp.br
...
```bash
[you@laptop:~]$ scp -p 6868  -r amdahl yourUsername@ssh.lsc.ic.unicamp.br:
```
...
```bash
[you@laptop:~]$ rsync --port=6868 -avP amdahl.tar.gz yourUsername@ssh.lsc.ic.unicamp.br:
```

The options are:

- `--port` to specify a custom SSH port (needed because we use 6868 as the SSH port in Sorgan, which is not the default port 22).
...
```bash
[you@laptop:~]$ rsync --port=6868 -avP amdahl yourUsername@ssh.lsc.ic.unicamp.br:~/
```
...
```bash
[you@laptop:~]$ rsync --port=6868 -avP yourUsername@ssh.lsc.ic.unicamp.br:amdahl ./
```
...
- Host: `sftp://ssh.lsc.ic.unicamp.br`
- User: Your cluster username
- Password: Your cluster password
- Port: 6868 (the SSH port for Sorgan)
