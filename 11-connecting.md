...
access. For systems like `ssh` running an SSH server, anybody
...
```bash
[you@laptop:~]$ scp -P 6868 ~/.ssh/id_ed25519.pub yourUsername@ssh.lsc.ic.unicamp.br:~/
```
...
- `[yourUsername@ssh ~]$` when the command is to be entered on a
  terminal connected to the remote login node system
