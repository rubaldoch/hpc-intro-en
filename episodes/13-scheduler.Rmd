...
```bash
`r config$remote$head_prompt` nano example-job.sh
```
...
```bash
`r config$remote$head_prompt` bash example-job.sh
```

```output
This script is running on `r config$remote$head_host`
```
...
```bash
`r config$remote$head_prompt` `r config$sched$submit$name` `r config$sched$submit$options` example-job.sh
```
...
```bash
`r config$remote$head_prompt` cat example-job.sh
```
...
```bash
`r config$remote$head_prompt` cat slurm-13937.out
```
...
```bash
`r config$remote$head_prompt` `r config$sched$del` 13938
...
```bash
`r config$remote$head_prompt` `r config$sched$del` -u `r config$remote$user`
```
...
```bash
`r config$remote$head_prompt` `r config$sched$interactive` hostname
```
...
```bash
`r config$remote$head_prompt` `r config$sched$interactive` -n 2 echo "This job will use 2 CPUs."
```
...
```bash
`r config$remote$head_prompt` `r config$sched$interactive` --pty bash
```
...
the `ssh` command, e.g., `ssh -X `r if (!is.null(config$remote$login_port) && nzchar(config$remote$login_port)) paste0("-p ", config$remote$login_port, " ") else ""``r config$remote$user`@`r config$remote$login_domain``.
