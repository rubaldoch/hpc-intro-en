...
```bash
`r config$remote$head_prompt` echo $PATH
```

```output
/export/spack/opt/spack/linux-rocky9-x86_64_v3/gcc-11.5.0/python-3.13.1-l4e6fblxgvpiiokcstye4x7puxi4722q/bin:/home/`r config$remote$user`/.local/bin:/export/spack/bin:/usr/local/bin:/usr/bin:/usr/local/sbin:/usr/sbin
```
...
```bash
`r config$remote$head_prompt` nano python-module.sh
`r config$remote$head_prompt` cat python-module.sh
```
...
```bash
`r config$remote$head_prompt` `r config$sched$submit$name` `r config$sched$submit$options` python-module.sh
```
