...
access. For systems like ``r config$remote$login_host`` running an SSH server, anybody
...
port_flag <- if (!is.null(config$remote$login_port) && nzchar(config$remote$login_port)) {
  paste0("-P ", config$remote$login_port, " ")
} else {
  ""
}
...
- ``r config$remote$login_prompt`` when the command is to be entered on a
...
```bash
`r config$local$prompt` ssh `r if (!is.null(config$remote$login_domain) && nzchar(config$remote$login_port)) paste0("-p ", config$remote$login_port, " ") else ""``r config$remote$user`@`r config$remote$login_domain`
```
