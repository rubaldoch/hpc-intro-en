...
```bash
`r config$local$prompt` mv hpc-carpentry-amdahl-d5a3bb9 amdahl
```
...
```bash
`r config$local$prompt` scp `r if (!is.null(config$remote$login_port) && nzchar(config$remote$login_port)) paste0("-P ", config$remote$login_port, " ") else ""` local_file `r config$remote$user`@`r config$remote$login_domain`:remote_destination
```
...
```bash
`r config$local$prompt` scp `r if (!is.null(config$remote$login_port) && nzchar(config$remote$login_port)) paste0("-p ", config$remote$login_port, " ") else ""` amdahl.tar.gz `r config$remote$user`@`r config$remote$login_domain`:
```

::: callout
## Shared home directory on `r config$remote$name`

On `r config$remote$name` the login node and the head node share the same `$HOME` directory. This means that any file you upload to your home directory on the login node will be available on the head node, and vice versa. This is not always the case on other clusters, so be sure to check with your system administrators if you are unsure about how your cluster is configured.
...
```bash
`r config$local$prompt` ssh `r config$remote$user`@`r config$remote$login_domain`
...
```bash
`r config$local$prompt` scp `r if (!is.null(config$remote$login_port) && nzchar(config$remote$login_port)) paste0("-p ", config$remote$login_port, " ") else ""` -r amdahl `r config$remote$user`@`r config$remote$login_domain`:
```
...
```bash
`r config$local$prompt` rsync `r if (!is.null(config$remote$login_port) && nzchar(config$remote$login_port)) paste0("--port=", config$remote$login_port, "") else ""` -avP amdahl.tar.gz `r config$remote$user`@`r config$remote$login_domain`:
```

The options are:

```{r, eval=!is.null(config$remote$login_port), echo=FALSE, results="asis"}
cat(
  paste0(
    "- `--port` to specify a custom SSH port (needed because we use ",config$remote$login_port,
    " as the SSH port in ",
    config$remote$name, 
    ", which is not the default port 22).",
    "\n"
  )
)
```
...
```bash
`r config$local$prompt` rsync `r if (!is.null(config$remote$login_port) && nzchar(config$remote$login_port)) paste0("--port=", config$remote$login_port, "") else ""` -avP amdahl `r config$remote$user`@`r config$remote$login_domain`:~/
```
...
```bash
`r config$local$prompt` rsync `r if (!is.null(config$remote$login_port) && nzchar(config$remote$login_port)) paste0("--port=", config$remote$login_port, "") else ""` -avP `r config$remote$user`@`r config$remote$login_domain`:amdahl ./
```
...
- Host: `sftp://`r config$remote$login_domain``
- User: Your cluster username
- Password: Your cluster password
- Port: `r config$remote$login_port` (the SSH port for `r config$remote$name`)
