...
```bash
`r config$remote$login_prompt` ssh `r config$remote$user`@`r config$remote$head_host`
``` 
...
In `r config$remote$name`, there is no direct access to the cluster head node from outside, so you must first log in to the login node and then connect to the head node.
If you are inside the `r config$organization` network, you can log in to the head node directly.

```bash
`r config$local$prompt` ssh `r config$remote$user`@`r config$remote$head_domain`
```
:::
...
```bash
`r config$remote$head_prompt` hostname
```

```output
`r config$remote$head_host`
```
...
```bash
`r config$remote$head_prompt` pwd
```
...
```bash
`r config$remote$head_prompt` ls -a
```
...
```bash
`r config$remote$head_prompt` `r config$sched$info`
```
...
```bash
`r config$remote$head_prompt` exit
`r config$remote$login_prompt` exit
...
`r config$remote$head_prompt` nproc --all
`r config$remote$head_prompt` free -m
```
...
```bash
`r config$remote$head_prompt` less /proc/meminfo
```
...
```bash
`r config$remote$head_prompt` df -Th
```
::::
:::
